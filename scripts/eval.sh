...
# Evaluate Script
# Usage: ./scripts/eval.sh [devices] [model_type] [size] [dataset] [prompt_masking]
# Example: ./scripts/eval.sh 0,1,2,3 qwen 0.6B thinksafe true
...
math_pids=()
safety_pids=()
log_dir="logs/eval_${model_type}-${size}-${DS}-${rank}-${epoch}ep"
mkdir -p "$log_dir"
    ...
    for device in "${device_array[@]}"; do
        ...
        if [ -z "${gpu_pids[$device]}" ] || ! kill -0 ${gpu_pids[$device]} 2>/dev/null; then
            ...
            if [ $task_idx -lt ${#math_tasks[@]} ]; then
                ...
                bash scripts/${model_type}/inference.sh "$device" "$size" "$task" "$DS" "$prompt_masking" "$rank" "$use_kl" "$epoch" > "$log_file" 2>&1 &
                pid=$!
    ...
    for device in "${device_array[@]}"; do
        ...
        if [ -z "${gpu_pids[$device]}" ] || ! kill -0 ${gpu_pids[$device]} 2>/dev/null; then
            ...
            if [ $task_idx -lt ${#safety_modes[@]} ]; then
                ...
                bash scripts/${model_type}/inference_safety.sh "$device" "$size" "$DS" "$mode" "$prompt_masking" "$rank" "$use_kl" "$epoch" > "$log_file" 2>&1 &
                pid=$!
...
echo "Results saved in: results/${model_type}-${size}-${DS}/"
