...
def evaluate(
    ...
    log_parts = [
        f"Temperature: {sampling_params.params.temperature}",
        f"top_p: {sampling_params.params.top_p}",
    ]
    if hasattr(sampling_params.params, 'top_k') and sampling_params.params.top_k is not None:
        log_parts.append(f"top_k: {sampling_params.params.top_k}")
    log_parts.append(f"max_tokens: {sampling_params.params.max_tokens}")
